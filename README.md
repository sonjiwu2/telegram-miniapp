...
- `POST /api/v1/sessions/:id/finalize` — идемпотентная финализация с реальной
  логикой пока только для `ROULETTE` (криптографически стойкий случайный
  выбор, `crypto.randomInt`). Для остальных типов — `501 FINALIZE_NOT_IMPLEMENTED`
  до соответствующего этапа.
...
Первый живой режим поверх Sessions API: `/who-today` — выбор/ввод вопроса,
добавление участников, `КРУТИТЬ` → сервер уже знает победителя (не фронтенд),
анимация просто визуализирует уже готовый результат. `Реванш` создаёт новый
независимый раунд с теми же участниками, `Новый вопрос` — полный сброс.
